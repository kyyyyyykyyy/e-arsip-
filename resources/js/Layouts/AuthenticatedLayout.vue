    ...
    <div class="min-h-screen bg-slate-50 flex">
        ...
        <aside class="hidden md:flex flex-col w-64 bg-slate-900 text-slate-300 shadow-2xl transition-all duration-300 z-20">
            <div class="h-16 flex items-center justify-center bg-slate-950 border-b border-slate-800">
                <span class="text-2xl font-extrabold text-amber-500 tracking-wider">SI<span class="text-white">DEPPA</span></span>
            </div>
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
                ...
                <div>
                    <button @click="isArsipOpen = !isArsipOpen" class="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-800 hover:text-white rounded-md transition-all duration-200 border-l-4 border-transparent">
                        <div class="flex items-center">
                            ...
                            <span class="font-medium">Pangkalan Data</span>
                        </div>
                    ...
                    <div v-show="isArsipOpen" class="mt-2 space-y-1 px-4 border-l-2 border-slate-700 ml-6">
                        <Link :href="route('arsip.index')" class="block px-4 py-2 text-sm text-slate-400 hover:text-amber-500 hover:bg-slate-800 rounded-md">Semua Dokumen</Link>
                        <Link :href="route('arsip.index', { category: 'DPA' })" class="block px-4 py-2 text-sm text-slate-400 hover:text-amber-500 hover:bg-slate-800 rounded-md">Arsip DPA</Link>
                        <Link :href="route('arsip.index', { category: 'RKA' })" class="block px-4 py-2 text-sm text-slate-400 hover:text-amber-500 hover:bg-slate-800 rounded-md">Arsip RKA</Link>
                        <Link :href="route('arsip.index', { category: 'Renja' })" class="block px-4 py-2 text-sm text-slate-400 hover:text-amber-500 hover:bg-slate-800 rounded-md">Arsip Renja</Link>
                        <Link :href="route('arsip.index', { category: 'Laporan Bulanan' })" class="block px-4 py-2 text-sm text-slate-400 hover:text-amber-500 hover:bg-slate-800 rounded-md">Laporan Kinerja</Link>
                    </div>
                </div>
                ...
                <Link :href="route('laporan.index')" :class="route().current('laporan.index') ? 'bg-slate-800 text-amber-500 border-l-4 border-amber-500' : 'hover:bg-slate-800 hover:text-white border-l-4 border-transparent'" class="flex items-center px-4 py-3 rounded-r-md transition-all duration-200 group">
                    ...
                    <span class="font-medium">Laporan Otomatis</span>
                </Link>
                ...
                <div v-if="user.role === 'admin'">
                    ...
                    <Link :href="route('users.index')" :class="route().current('users.index') ? 'bg-slate-800 text-amber-500 border-l-4 border-amber-500' : 'hover:bg-slate-800 hover:text-white border-l-4 border-transparent'" class="flex items-center px-4 py-3 rounded-r-md transition-all duration-200">
                        ...
                        <span class="font-medium">Kelola User / Bidang</span>
                    </Link>
                    ...
                    <Link :href="route('logs.index')" :class="route().current('logs.index') ? 'bg-slate-800 text-amber-500 border-l-4 border-amber-500' : 'hover:bg-slate-800 hover:text-white border-l-4 border-transparent'" class="flex items-center px-4 py-3 rounded-r-md transition-all duration-200">
                        ...
                        <span class="font-medium">Log Aktivitas CCTV</span>
                    </Link>
        ...
        <div class="flex-1 flex flex-col min-h-screen transition-all duration-300">
            <header class="h-16 bg-white shadow-sm border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 lg:px-8 z-10 relative">
                ...
                <div class="hidden md:flex flex-col">
                    <span class="text-sm font-bold text-slate-800">Pangkalan Data SIDEPPA</span>
                    <span class="text-xs text-slate-500">Sistem Informasi Digital Perencanaan dan Pelaporan</span>
                </div>
                                ...
                                <span class="inline-flex rounded-md">
                                    <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-slate-700 bg-white hover:text-amber-600 focus:outline-none transition ease-in-out duration-150">
                                        <div class="flex flex-col text-right mr-2">
                                            ...
                                            <span class="text-xs text-amber-600 uppercase tracking-wider font-bold">{{ user.role }} SIDEPPA</span>
                                        </div>
            ...
            <div :class="{'block': showingNavigationDropdown, 'hidden': !showingNavigationDropdown}" class="md:hidden bg-slate-900 border-b border-slate-800 absolute w-full z-50 top-16 shadow-xl">
                <div class="pt-2 pb-3 space-y-1">
                    <Link :href="route('dashboard')" :class="route().current('dashboard') ? 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-amber-500 bg-slate-800 border-amber-500' : 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-slate-300 border-transparent hover:bg-slate-800 hover:text-white'">Dashboard Utama</Link>
                    <Link :href="route('arsip.index')" :class="route().current('arsip.index') ? 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-amber-500 bg-slate-800 border-amber-500' : 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-slate-300 border-transparent hover:bg-slate-800 hover:text-white'">Pangkalan Data</Link>
                    <Link :href="route('laporan.index')" :class="route().current('laporan.index') ? 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-amber-500 bg-slate-800 border-amber-500' : 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-slate-300 border-transparent hover:bg-slate-800 hover:text-white'">Laporan Otomatis</Link>
                    
                    <div v-if="user.role === 'admin'">
                        <Link :href="route('users.index')" :class="route().current('users.index') ? 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-amber-500 bg-slate-800 border-amber-500' : 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-slate-300 border-transparent hover:bg-slate-800 hover:text-white'">Kelola User / Bidang</Link>
                        <Link :href="route('logs.index')" :class="route().current('logs.index') ? 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-amber-500 bg-slate-800 border-amber-500' : 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-slate-300 border-transparent hover:bg-slate-800 hover:text-white'">Log Aktivitas CCTV</Link>
                        <Link :href="route('settings.index')" :class="route().current('settings.index') ? 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-amber-500 bg-slate-800 border-amber-500' : 'block pl-3 pr-4 py-2 border-l-4 text-base font-medium text-slate-300 border-transparent hover:bg-slate-800 hover:text-white'">Pengaturan Sistem</Link>
                    </div>
                </div>
            </div>
