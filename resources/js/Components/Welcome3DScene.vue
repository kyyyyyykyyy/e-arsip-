...
onMounted(() => {
    ...
    const dirLight = new THREE.DirectionalLight(0x38bdf8, 4); // Cahaya Biru Cyan
    dirLight.position.set(-5, 10, 5);
    scene.add(dirLight);
    
    const pointLight = new THREE.PointLight(0xf59e0b, 8, 20); // Cahaya Emas/Amber
    ...
    const particleMat = new THREE.PointsMaterial({ size: 0.05, color: 0x38bdf8, transparent: true, opacity: 0.8 });
    ...
    const animate = () => {
        ...
        coreGroup.position.y = Math.sin(elapsed * 2) * 0.3; // Gerak melayang
        innerCore.rotation.y -= delta * 0.8; // Inti berputar berlawanan arah
...
<template>
    <div class="absolute inset-0 w-full h-full bg-slate-950 -z-10 overflow-hidden">
        ...
        <div class="absolute inset-0 bg-gradient-to-b from-slate-950/60 via-transparent to-slate-950 pointer-events-none z-0"></div>
    </div>
</template>