...
const totalMasuk = computed(() => props.documents ? props.documents.filter(d => d.category === 'Surat Masuk').length : 0);
const totalKeluar = computed(() => props.documents ? props.documents.filter(d => d.category === 'Surat Keluar').length : 0);
const totalInternal = computed(() => props.documents ? props.documents.filter(d => d.category === 'Internal').length : 0);
...
const form = useForm({
    ...
    document_date: '',
    origin_office: '',      // Khusus Surat Masuk
    destination_office: '', // Khusus Surat Keluar
    destination_field: '',  // Khusus Internal
    description: '',
    file: null,
});
...
const submitArsip = () => {
    form.post(route('documents.store'), {
        ...
        onSuccess: () => {
            closeModal();
            // Data dan grafik otomatis update berkat reaktivitas Vue!
        },
    });
};
// ==========================================
        ...
        <div class="max-w-7xl mx-auto overflow-hidden p-2 relative">
            ...
            <div :class="pageLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'" class="transition-all duration-700 ease-out mb-8 flex flex-col md:flex-row md:items-center md:justify-between bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                <div>
                    ...
                    <p class="text-slate-500 mt-1 text-sm">
                        Berikut adalah ringkasan aktivitas dokumen dan performa arsip digital Anda.
                    </p>
                </div>
            ...
            <div :class="pageLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'" class="transition-all duration-700 delay-100 ease-out grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div class="relative overflow-hidden bg-gradient-to-br from-blue-500 to-blue-700 rounded-2xl p-6 text-white group cursor-default transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/40 hover:scale-[1.02]">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-blue-100 text-sm font-semibold uppercase tracking-wider mb-1">Total Arsip</p>
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-2xl p-6 text-white group cursor-default transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-emerald-500/40 hover:scale-[1.02]">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-emerald-100 text-sm font-semibold uppercase tracking-wider mb-1">Surat Masuk</p>
                            <h3 class="text-4xl font-extrabold transition-all duration-300 group-hover:scale-110 origin-left">{{ totalMasuk }}</h3>
                        </div>
                        <div class="p-3 bg-white/20 rounded-xl backdrop-blur-sm group-hover:rotate-12 transition-transform duration-300">
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl p-6 text-white group cursor-default transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-amber-500/40 hover:scale-[1.02]">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-amber-100 text-sm font-semibold uppercase tracking-wider mb-1">Surat Keluar</p>
                            <h3 class="text-4xl font-extrabold transition-all duration-300 group-hover:scale-110 origin-left">{{ totalKeluar }}</h3>
                        </div>
                        <div class="p-3 bg-white/20 rounded-xl backdrop-blur-sm group-hover:rotate-12 transition-transform duration-300">
                ...
                <div class="relative overflow-hidden bg-gradient-to-br from-purple-500 to-indigo-700 rounded-2xl p-6 text-white group cursor-default transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-purple-500/40 hover:scale-[1.02]">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-purple-100 text-sm font-semibold uppercase tracking-wider mb-1">Internal</p>
                            <h3 class="text-4xl font-extrabold transition-all duration-300 group-hover:scale-110 origin-left">{{ totalInternal }}</h3>
                        </div>
                        <div class="p-3 bg-white/20 rounded-xl backdrop-blur-sm group-hover:rotate-12 transition-transform duration-300">
            ...
            <div :class="pageLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'" class="transition-all duration-700 delay-200 ease-out bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 hover:shadow-md">
                <div class="flex justify-between items-center mb-8">
                    <h3 class="text-lg font-bold text-slate-800">Aktivitas Pengarsipan ({{ new Date().getFullYear() }})</h3>
                            ...
                            <form @submit.prevent="submitArsip" class="space-y-5">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <input v-model="form.document_number" type="text" class="w-full border-slate-300 rounded-lg focus:ring-amber-500 focus:border-amber-500 text-sm" placeholder="Contoh: 001/SP/2026" required>
                                ...
                                <div>
                                    ...
                                    <input v-model="form.title" type="text" class="w-full border-slate-300 rounded-lg focus:ring-amber-500 focus:border-amber-500 text-sm" placeholder="Masukkan perihal dokumen..." required>
                                ...
                                <transition name="fade" mode="out-in">
                                    <div v-if="form.category === 'Surat Masuk'" key="masuk">
                                        <label class="block text-sm font-semibold text-emerald-700 mb-1">Asal Surat Instansi</label>
                                        <input v-model="form.origin_office" type="text" class="w-full border-emerald-200 bg-emerald-50 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 text-sm" placeholder="Dikirim dari instansi mana?">
                                    </div>

                                    <div v-else-if="form.category === 'Surat Keluar'" key="keluar">
                                        <label class="block text-sm font-semibold text-amber-700 mb-1">Tujuan Instansi</label>
                                        <input v-model="form.destination_office" type="text" class="w-full border-amber-200 bg-amber-50 rounded-lg focus:ring-amber-500 focus:border-amber-500 text-sm" placeholder="Dikirim ke instansi mana?">
                                    </div>

                                    <div v-else-if="form.category === 'Internal'" key="internal">
                                        <label class="block text-sm font-semibold text-purple-700 mb-1">Tujuan Bidang</label>
                                        <input v-model="form.destination_field" type="text" class="w-full border-purple-200 bg-purple-50 rounded-lg focus:ring-purple-500 focus:border-purple-500 text-sm" placeholder="Contoh: Bidang Penegakan Perda">
                                    </div>
                                </transition>
                                <div>
